      ...
      >
        ...
        <div className="relative z-10 max-w-5xl mx-auto flex flex-col items-center">
          ...
          <nav className="flex flex-wrap justify-center items-center gap-3">
            <Link href="/presentation" className="bg-white/20 backdrop-blur-sm text-white px-5 py-2 rounded-full border border-white/30 hover:bg-white/30 transition text-sm font-medium">
              Présentation
            </Link>
            <Link href="/activites" className="bg-white/20 backdrop-blur-sm text-white px-5 py-2 rounded-full border border-white/30 hover:bg-white/30 transition text-sm font-medium">
              Activités
            </Link>
            <Link href="/agenda" className="bg-white/20 backdrop-blur-sm text-white px-5 py-2 rounded-full border border-white/30 hover:bg-white/30 transition text-sm font-medium">
              Agenda
            </Link>
            {SITE_INFOS.lienMedia && (
              <a href={SITE_INFOS.lienMedia} target="_blank" rel="noopener noreferrer" className="bg-amber-500 text-amber-950 px-5 py-2 rounded-full hover:bg-amber-400 transition text-sm font-bold shadow-md">
                🎬 Médias
              </a>
            )}
          </nav>
      ...
      {NEWS_INFO.afficherSurAccueil && (
        <section className="max-w-4xl mx-auto mt-12 p-6 bg-amber-50 rounded-2xl border border-amber-200 shadow-sm">
          <div className="flex flex-col md:flex-row gap-6 items-center">
            {NEWS_INFO.image && (
              <img src={NEWS_INFO.image} alt={NEWS_INFO.titre} className="w-full md:w-48 h-32 object-cover rounded-xl shadow-md" />
            )}
            <div className="flex-1 text-center md:text-left">
              ...
              {NEWS_INFO.lien && (
                <a href={NEWS_INFO.lien} target="_blank" rel="noopener noreferrer" className="inline-block text-sm font-bold text-amber-700 hover:underline">
                  En savoir plus →
                </a>
              )}
            </div>
